import numpy as np
from lops.signalprocessing import Convolve2D
...
def Smoothing2D(nsmooth, dims, nodir=None, dtype='float32'):
    ...
    depending on the choice of ``nodir``.
    ...
    Notes
    -----
    The 2D Smoothing operator is a special type of convolutional operator that convolves
    the input model (or data) with a constant 2d filter of size
    :math:`n_{smooth, 1} \quad x \quad n_{smooth, 2}`:

    Its application to a two dimensional input signal is:

    .. math::
        y[i,j] = 1/(n_{smooth, 1}*n_{smooth, 2}) \sum_{l=-(n_{smooth,1}-1)/2}^{(n_{smooth,1}-1)/2}
    ...
    """
    if isinstance(nsmooth, tuple): nsmooth=list(nsmooth)
    nsmooth[0] = nsmooth[0] + 1 if nsmooth[0] % 2 == 0 else nsmooth[0]
    nsmooth[1] = nsmooth[1] + 1 if nsmooth[1] % 2 == 0 else nsmooth[1]

    return Convolve2D(np.prod(np.array(dims)),
