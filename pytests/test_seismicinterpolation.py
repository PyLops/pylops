...
par1_3d = par1_2d
par2_3d = {'kind': 'fk',
           'kwargs': dict(nffts=(2**7, 2**7, 2**8), returninfo=False,
                          sampling=(par['dy'], par['dx'], par['dt']),
                          niter=20, eps=5e-2, alpha=1e0, show=False)}
par3_3d = {'kind': 'radon-linear',
           'kwargs': dict(paxis=np.linspace(-1e-3, 1e-3, 21),
                          p1axis=np.linspace(-1e-3, 1e-3, 50), centeredh=True,
                          niter=20, eps=1e-3,
                          alpha=1.3e-6, show=False)}
par4_3d = {'kind': 'sliding',
           'kwargs': dict(paxis=np.linspace(-1e-3, 1e-3, 21),
                          p1axis=np.linspace(-1e-3, 1e-3, 21),
                          nwin=(12, 5), nwins=(3, 2), nover=(3, 2),
                          design=True, niter=20, eps=1e-2,
                          alpha=1.3e-4, show=False)}
par1_3d.update(par)
par2_3d.update(par)
par3_3d.update(par)
par4_3d.update(par)
...
def test_SeismicInterpolation3d(par):
    ...
    if par['kind'] == 'sliding':
        ...
        win0end = (par['kwargs']['nwin'][0] - par['kwargs']['nover'][0]) * \
                  par['kwargs']['nwins'][0] - par['kwargs']['nover'][0]
        win1in = par['kwargs']['nover'][1]
        win1end = (par['kwargs']['nwin'][1] - par['kwargs']['nover'][1]) * \
                  par['kwargs']['nwins'][1] - par['kwargs']['nover'][1]
