...
par4 = {'nt': 21, 'nx': 101, 'dt': .3,
        'imag': 0, 'dtype': 'float64'} # odd samples, real, non-unitary step
...
def test_CausalIntegration1d(par):
    ...
    for kind, rf in itertools.product(('full', 'half', 'trapezoidal'),
        ...
        Cop = CausalIntegration(par['nt'], sampling=par['dt'],
                                kind=kind, halfcurrent=False,
                                removefirst=rf,
                                dtype=par['dtype'])
        rf1 = 1 if rf else 0
        assert dottest(Cop, par['nt'] - rf1, par['nt'],
...
def test_CausalIntegration2d(par):
    ...
    for kind, rf in itertools.product(('full', 'half', 'trapezoidal'),
        ...
        assert dottest(Cop, (par['nt'] - rf1)*par['nx'], par['nt']*par['nx'],
                       complexflag=0 if par['imag'] == 0 else 3)
